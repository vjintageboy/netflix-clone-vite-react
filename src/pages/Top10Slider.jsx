...
export default function Top10Slider({ items, title = "Top 10 Movies" }) {
  ...
  const handleMovieHover = (item, index) => {
    ...
    // Set timer for showing dialog after 1 second
  ...
  if (!items || items.length === 0) {
    return <div className="text-white p-4">Loading Top 10 movies...</div>;
  }

  return (
    <div className="group relative w-full mt-15 mx-auto flex flex-col items-center" style={{height: '220px'}}>
      ...
      <div className="absolute top-0 left-16 p-0 z-12 text-[1.4vw] font-black text-white" style={{ margin: '-30px 0 12px', textShadow: '2px 2px 4px rgba(0,0,0,0.8)', fontWeight: '900' }}>
       {title}
      </div>
        ...
        >
          {items.map((item, index) => (
            ...
            >
              ...
              {/* Movie Preview Dialog - Inline */}
              {showDialog && selectedMovie && hoveredIndex === index && (
                <div 
                  className="absolute z-50 bg-gray-900 rounded-lg w-80 shadow-2xl border border-gray-700"
                  style={{
                    top: '-10px',
                    left: '50%',
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <div className="text-gray-300 text-xs space-y-1">
                          <p><span className="text-white font-medium">Xếp hạng:</span> #{selectedMovie.rank}</p>
